...
export default class ValidateRegistrationPage extends CommonValidator {
  ...
  public validateInput(): boolean {
    ...
    switch (this.name) {
      ...
      case 'second_name':
        if (
          /^[A-ZА-ЯЁ][a-zA-Zа-яёА-ЯЁ-]*[a-zA-Zа-яёА-ЯЁ-]$/.test(inputValue) ===
          false
        ) {
          this.check = false;
          this.errorTxt =
            'Поле должно быть заполнено латиницей или кириллицей, первая буква должна быть заглавной, без пробелов и без цифр, нет спецсимволов';
        }
        break;
      case 'login':
        if (
          this.checkLength(inputValue, 3, 20) ||
          /[a-zA-Z-_]/.test(inputValue) === false ||
          /[а-яА-ЯёЁ_-]/.test(inputValue) === true
        ) {
          this.check = false;
          this.errorTxt =
            'Условия не соблюдены: от 3 до 20 символов, латиница, может содержать цифры, но не состоять из них, без пробелов, без спецсимволов (допустимы дефис и нижнее подчёркивание)';
        }
        break;
      case 'password':
        if (
      ...
      case 'email':
        if (
          /^[a-zA-Z0-9_-]+@[a-zA-Z]+\.[a-zA-Z]+$/.test(inputValue) === false
        ) {
          this.check = false;
          this.errorTxt =
            'Условия не соблюдены: латиница, может включать цифры и спецсимволы вроде дефиса и подчёркивания, обязательно должна быть «собака» (@) и точка после неё, но перед точкой обязательно должны быть буквы.';
        }
        break;
      case 'phone':
        if (
          this.checkLength(inputValue, 10, 15) ||
          /^[0-9+][0-9]/.test(inputValue) === false
        ) {
          this.check = false;
          this.errorTxt =
            'Условия не соблюдены: от 10 до 15 символов, состоит из цифр, может начинается с плюса.';
        }
        break;
      default:
        this.check = true;
